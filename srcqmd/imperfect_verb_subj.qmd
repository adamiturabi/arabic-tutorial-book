...
The a-state ;mudarie; verb implies a wish or purpose.
The a-state ;mudarie; verb is used after the following articles:
...
## Forming the a-state ;mudarie; verb
...
|Participant|;_mudarie; verb ;fail; pronoun|u-state ;mudarie; verb | a-state ;mudarie; verb|
|:---|:--|:---|:---|
...
+ The ;ustate; and a-state verbs are the same for the feminine plural absentee and addressee participants: 
...
;mudarie; verb to be in the a-state.
...
### Basic usage of [أَنْ]{.ar} [Ean]{.trn} with the a-state ;mudarie; verb
...
[لَا]{.ar} can be used to negate the following a-state ;mudarie; verb.
...
[أَنْ]{.ar} must directly precede the following a-state ;mudarie; verb and must not be separated from it.
...
In the i-state as the ;mudafil; in an ;idafa;:
...
\draw [decoration={brace, mirror}, decorate] 
   (word-2.south west) -- (word-2.south east)
      node [midway, below] {\scriptsize i-state ;mudafil;};
...
<!--
By the way, this [لِ]{.ar}, which is used with a-state ;mudarie; verbs, is different from 
the [لِ]{.ar} we learned in section\ \@ref(indirect-commands), which is used with the 0-state ;mudarie; verbs for indirect commands.
-->

By the way, the grammatical equivalence of [أَنْ]{.ar} and a following a-state ;mudarie; verb with a ;masdar; applies also to when [لِ]{.ar} is used before (either an expressed or an implied) [أَنْ]{.ar}. So, for example, if we have a sentence:
...
There is a specific [لِ]{.ar}, called the [لِ]{.ar} of denial,  which is used with a-state ;mudarie; verbs and the verb [كَانَ]{.ar} that we will discuss in section (TODO in [كَانَ]{.ar} chapter).
...
It is also used before the a-state ;mudarie; verb.
The difference from [لِ]{.ar} is that, when [لِ]{.ar} is used with the a-state ;mudarie; verb, expressing or dropping the [أَنْ]{.ar} was optional.
...
By the way, 
[كَيْ]{.ar} and a following a-state ;mudarie; verb are not _directly_ replaced by
a ;masdar;.
...
[رَأْس]{.ar},
here, is in the a-state because it is a ;mafulb; of the verb [أَكَلَ]{.ar} "ate".
The particle
[حَتَّىٰ]{.ar} "even",
...
#### [حَتَّىٰ]{.ar} with a following a-state ;mudarie; verb

When
[حَتَّىٰ]{.ar}
is used with an expectation or purpose of a future action of the verb following it, then 
the verb following it is an a-state ;mudarie; verb. 
<!--
When the verb following
[حَتَّىٰ]{.ar}
signifies the expectation or purpose of a future action, then the verb used is an a-state ;mudarie; verb. 
-->
...
1. When [حَتَّىٰ]{.ar} is used to indicate an extreme point at which the action of the following verb would occur, or is meant to occur. Here, 
   ...
   Also, similar to the case of [كَيْ]{.ar}, there is an assumed (but mandatorily unexpressed) [أَنْ]{.ar} which is the real cause of the following ;mudarie; verb being in the a-state.
   In fact, 
   [حَتَّىٰ]{.ar},
...
2. When [حَتَّىٰ]{.ar} is used with the meaning "to such a purpose that". This is a similar meaning to [كَيْ]{.ar} "in order that".
   ...
   Again, there is an assumed (but mandatorily unexpressed) [أَنْ]{.ar} which is the real cause of the following ;mudarie; verb being in the a-state.
...
Compare these examples with the corresponding ones in the previous sub-section that have an a-state ;mudarie; verb.


<!--The preposition 
[حَتَّىٰ]{.ar} [HattA]{.trn}
is unusual in that the ;mudarie; verb following it may either be in the a-state or the ;ustate;, with slightly different meanings.
When the ;mudarie; verb following it is in the ;ustate; then
[حَتَّىٰ]{.ar} gives the meaning "until".
And when the ;mudarie; verb following it is in the a-state then
...
[لَا]{.ar} and [أَنْ]{.ar} are combined to form [لَنْ]{.ar} [lan]{.trn} with the meaning "shall not". [لَنْ]{.ar} is used with the a-state ;mudarie; verb to emphatically negate the future.
...
occur after an a-state ;mudarie; verb, then 
a second a-state ;mudarie; verb
(that doesn't have its own [أَنْ]{.ar}, etc.)
may be either in the a-state or the ;ustate;.
For example,
...
[وَ]{.ar}, [فَ]{.ar}, [أَوْ]{.ar}, and [ثُمَّ]{.ar} also cause the following ;mudarie; verb to be in the a-state in their own right, not simply as connectors. This is discussed in more detail in chapter TODO.
