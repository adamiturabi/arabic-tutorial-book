...
2. The adjective "big" can also be used as the ;xabar; of a sentence, describing the subject noun. For example:
...
Instead, here we will continue to use the existing terminology of _subject_ and _;xabar;_. The definite noun "the car" is the subject of this sentence, and the adjective "big" is the ;xabar;.
...
When an adjectival noun is the ;xabar; of a sentence, then it shall be genderized to match the gender of the subject noun. The subject noun in this case ([ٱَلْبَيْت]{.ar}) is masculine. Therefore, the masculine adjectival noun ([صَغِير]{.ar}) is chosen.
...
The solution, too, is the same. We insert a detached pronoun, that matches the gender of the subject, between the subject and the ;xabar;. So in order to get our intended meaning, we will say:
...
Note that the ;xabar; does not match the subject in gender. This is because it lost its genderizability since it is no longer acting as an adjectival noun "a good one~f~", but rather as the common noun "a good deed".
