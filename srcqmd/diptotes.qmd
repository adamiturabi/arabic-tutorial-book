...
As you can see, when [صَحْرَاء]{.ar} [SaHrAE]{.trn} is indefinite, it is not ;tanwin;ed. Also, when it is indefinite and in the i-state, the vowel mark on its final letter is not [◌ِ]{.ar}, as you might expect but [◌َ]{.ar} . And so the noun looks identical in the a-state and i-state when it is indefinite.
...
2. When indefinite and in the i-state, a semi-flexible noun's final letter does not have an ;imark;. Instead it shall have an ;amark;, just like when it is in the a-state.
