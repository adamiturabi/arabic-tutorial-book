...
We have already studied the u-state of ;mudarie; verbs in 
chapter\ \@ref(u-state-;mudarie;-verbs).
...
;mudarie; verb
ends with a 
0-mark [◌ْ]{.ar}, and the next word begins with a connecting [hamzah]{.trn2} [ٱ]{.ar} then the
0-mark [◌ْ]{.ar} is converted to an ;imark; [◌ِ]{.ar}. For example:
  + [يَفْعَلْ + ٱلرَّجُلُ = يَفْعَلِ ٱلرَّجُلُ]{.ar}

<!--
## Uses of the 0-state ;mudarie; verb

The u-state is the default state for ;mudarie; verbs. The 
0-state
...
<!--
FIXME: Use with the addressee is exceedingly rare. Rather the verb of command should be used (below). See [كتاب شرح المفصل لابن يعيش]{.ar} vol 4 p. 691. Add (perhaps in passive verbs chapter), how [لام الأمر]{.ar} can be used with passive verbs for all three deputy ;fail;s, including addressee, and is infact the only way to command the deputy ;fail;.
...
that [لَا]{.ar} can be used to negate a u-state ;mudarie; verb for the present and future tense.
Example:
...
In order to form the verb of command, we remove the initial [ت]{.ar} from the addressee particpant verb. The verb then begins with an 0-mark so we place a connecting [hamzah]{.trn2} in front of it.
