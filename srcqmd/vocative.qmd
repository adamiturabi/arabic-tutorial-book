...
When a specific person is called out to, and the name used to call him consists of a single word, then that word shall be in the ;ucase;. The sentence above is an example of this rule where the name [زَينَبُ]{.ar} [zaynabu]{.trn} "Zaynab" is in the ;ucase;.
...
In this case, then instead of the ;ucase;, the word is put into the ;acase;. Furthermore, the ;tanwin;, if any, is preserved. Examples:
...
If, instead of a personal name, a noun phrase consisting of multiple words is used to call out to a person, then in this case as well, the first noun shall be in the ;acase;. Examples:
...
When multiple words are used to call out to a person, the second word in the noun-chain may be a pronoun. Here too, the first noun shall be in the ;acase;. Examples:
...
If an unspecified person or persons are being called out, then the word used to call out is put into the ;acase;. Furthermore, the ;tanwin;, if any, is preserved. So if you want to address any unspecific Muslim, you will say:
...
If multiple words are used, whether or not the person called out to is specific or unspecified, then too the first noun is put in the ;acase;.
...
Note that even when [يَا]{.ar} [yA]{.trn} is ommitted the name [زَيْدُ]{.ar} [zaydu]{.trn} "Zayd" is in the ;ucase; without any ;tanwin;.
