...
And the ;mudarie; unknown-doer verb has the same three states as the ;mudarie; known-doer verb.
For example:
...
When such verbs are converted to the unknown-doer verb construction, then one, and only one, ;mafulb; shall be chosen to be the deputy doer. It is generally preferred to make the first ;mafulb; the deputy-doer, and leave the second ;mafulb;, as is, in the a-state.
...
When verbs are used without a ;mafulb;, then their unknown-doer construction gives in an impersonal meaning. There are a few such usages that we will explain in the following subsections.
...
When such sentences are converted to the unknown-doer verb construction, then the ;masdar; may be taken as the deputy doer in the u-state.
...
When a prepositional phrase, or an adverb of time or place, or a ;masdar; is to be used as a deputy doer in an unknown-doer verb construction, then it is required that they be specialized in meaning, and not used in a general sense. So for example, if we have the sentence:
...
## States of the ;mudarie; unknown-doer verb

The 
;mudarie; verb unknown-doer verb
