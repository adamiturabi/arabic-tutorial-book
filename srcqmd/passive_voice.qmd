...
Note that the noun [ٱَلْمَاءُ]{.ar} [EalmAEu]{.trn} "the water" is in the u-state. This is because, in the absence of the doer, the ;mafulb; of the known-doer verb ([ٱَلْمَاءَ]{.ar}) is deputized to take doer's place in the u-state. It is then known as the _deputy doer_.
...
### The deputy doer for multiple ;mafulb;s
...
When such verbs are converted to the ;passive; verb construction, then one, and only one, ;mafulb; shall be chosen to be the deputy doer. It is generally preferred to make the first ;mafulb; the deputy-doer, and leave the second ;mafulb;, as is, in the a-state.
...
However, the preposition causes the noun following it to remain in the i-state. So the deputy doer is not indicated by an apparent ;umark; (or by the other indicators of the u-state).
...
Other verbs don't take a ;mafulb; but may be used with an adverb of time or place in the a-state. 
...
When such sentences are converted to the ;passive; verb construction, then the ;masdar; may be taken as the deputy doer in the u-state.
...
When a prepositional phrase, or an adverb of time or place, or a ;masdar; is to be used as a deputy doer in an ;passive; verb construction, then it is required that they be specialized in meaning, and not used in a general sense. So for example, if we have the sentence:
...
Then only one shall be chosen as the deputy doer. If a ;mafulb; exists, then it is typically chosen. If a ;mafulb; does not exist, then the one desired to be emphasized may be chosen as the deputy doer. For example:
...
(Note that [يُغْفَرَ]{.ar} has a singular masculine deputy doer because there is no ;mafulb;.)
