...
We have learned that the essential meaning of the ;ismfail; is the doer of the action of the verb from which it is derived. In addition to this essential meaning, the ;ismfail; can also be used in place of the verb from which it is derived. This is only done when the verb to be replaced is the incomplete-action verb. The ;ismfail; does not replace the completed-action verb. We will now explain this usage.
...
If we wish to indicate that Zayd is actually going to school at present we can replace the incomplete-action verb with the indefinite ;ismfail;. So we get:
...
3. Quite similar to what we learned in section\ \@ref(the-direct-doee-in-i-state-preceded-by-the-preposition-%D9%84-li) for ;masdar;s, the ;mafulb; can follow the ;ismfail; in the i-state preceded by the preposition [لِ]{.ar} [li]{.trn}.
...
So far we have seen only an indefinite ;ismfail; being used with the meaning of an incomplete-action verb. However, the definite ;ismfail;, too, can give this meaning. The meaning is often in the present tense. Here are some examples:
...
Much of what has been said regarding the ;ismfail; applies to the ;ismmaful; as well: The ;ismmaful; may be used with a verbal meaning for the incomplete-action verb only. So if we say:
...
Doer and ;ismmaful;s are often re-used as adjectival-nouns with meanings that are directly formed from their doer and doee meaning respectively. Here are some examples:
...
The re-use of a ;ismfail; or ;ismmaful; as a common noun does not prevent it from being used with its doer/doee or verbal meaning any more. 
