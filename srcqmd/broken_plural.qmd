...
+ The letters [ء]{.ar}, [ا]{.ar}, [و]{.ar}, and [ي]{.ar} are considered _weak_ letters. Words that one or more these weak letters in their roots are called _defective_ words. We will discuss defective words more completely in later chapters, if [#allAh]{.trn2} wills. For now, we will note the following:
  ...
  + A [ي]{.ar} at the end of a word, in some ;case;s, gets omitted  and replaced by a ;tanwin;ed ;imark; [◌ٍ]{.ar} on the preceding letter. This happens even when the [ي]{.ar} is extrinsic to the root, and even if the word is semi-flexible (and thus would not normally be ;tanwin;ed). Examples:
...
Consistent with what we have learned so far, when the ;manut; in a noun-phrase is plural, then the ;na3t; comes after it, and matches it in ;case;, definiteness, gender, and number.
