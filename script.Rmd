...
Arabic is written from right-to-left, unlike English. When writing the letters in a word are generally connected to each other, except for six out of the 28 letters. These six letters are [ا، د، ذ، ر، ز، و]{.ar}. They are connected to the letter before them, like the rest of the letters, but they don't connect to the letter following them.

When connecting the letters, each letter is modified in order to connect to the preceding and following letter. Therefore, there can be up to four shapes for each letter: when the letter is:
...
So does that mean that we have to remember $4 \times 28 = 112$ different shapes? Not really. Remember that six of the letters don't connect to the following letter. So they only have isolated and end forms. And also, each letter maintains its basic shape and is usually only slightly modified in a common technique to connect to the previous and following letter. So if we remember this common technique of modifying the letter shape to connect it it will be easy to connect the letters.
...
You can see that each letter maintains a basic shape and is modified for each of the four positions. To explain the common technique of modifying the letters we will take [ب]{.ar} as an example and start with the isolated form:

Isolated form: [ب]{.ar}

To modify this into the end form, we simply connect a horizontal line to the right of the letter:
...
Now most of the letters follow this common technique but a few of them are modified a little further in each form. These, more complicated, letters are [ع، غ، ك، ه، ي]{.ar} and you can review them in the table above.

### Connecting the different forms to make a word

Notice that when we modified the isolated form to get to the beginning, middle, and end forms, we added a horizontal line to each or both sides. It is this horizontal line which connectes to the horizontal line of the neighboring letter.

As an example, we would like to connect the following letters (starting from the right): [م، ع، ش، ر]{.ar} into one word. The first letter is [م]{.ar} so we modify it to its beginning form [مـ]{.ar}. The next two letters are converted to their middle forms [ـعـ، ـشـ]{.ar}. And the last letter [ر]{.ar} is converted to its end form [ـر]{.ar}. Then we connect the horizontal lines together and get [معشد]{.ar}.

More complicated examples with no end-join letters
...
The letters [ب، ت، ث]{.ar} differ only in their dots and are otherwise identical in all positions. ن and ي are similar in initial and middle positions to ب ت ث but differ from them and from each other in isolated and final positions.
...
Thus each letter connects to the letters before and after it through this horizontal line.
