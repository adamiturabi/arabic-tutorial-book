...
the Arab grammarians were describing the Arabic language, drawing upon a corpus that included transmitted texts and the dialectal speech of informants.
...
While the grammar that we cover in this book is of [sca]{.smallcaps}, the vocabulary may include legitimate post-classical words and meanings.
...
Which can put the grammar of this definition of [msa]{.smallcaps} at odds with the grammar of [sca]{.smallcaps}.
...
We have also used some grammatical terminology from Arabic directly, usually in cases where our target learner may have already been exposed to them from [#qurEAn]{.trn2} recitation rules, or where the term is complex enough that creating a term specifically in English wouldn't make it any simpler.
...
In drawing material from sources, we haven't shied away from sticking close to the cited text when we felt we could not better its expression or organization for our purposes.
Citations are absent for basic topics, but also when we could not find suitable references for topics we wish to present.
