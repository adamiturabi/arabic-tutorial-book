...
\section{\texorpdfstring{The a-state incomplete-action verb after \foreignlanguage{arabic}{لِ} \emph{li}}{The a-state incomplete-action verb after لِ li}}\label{the-a-state-incomplete-action-verb-after-ux644-li}

The article \foreignlanguage{arabic}{أَنْ} may be attached to the preposition \foreignlanguage{arabic}{لِ} \emph{li} thus: \foreignlanguage{arabic}{لِأَنْ} \emph{liʾan} to give the meaning \enquote{so that}. For example:
...
\section{\texorpdfstring{The a-state incomplete-action verb after \foreignlanguage{arabic}{کَيْ} \emph{kay}}{The a-state incomplete-action verb after کَيْ kay}}\label{the-a-state-incomplete-action-verb-after-ux643ux64a-kay}
...
\section{\texorpdfstring{The a-state incomplete-action verb after \foreignlanguage{arabic}{حَتَّىٰ} \emph{ḥattā}}{The a-state incomplete-action verb after حَتَّىٰ ḥattā}}\label{the-a-state-incomplete-action-verb-after-ux62dux62aux649-hatta}
...
\section{\texorpdfstring{The a-state incomplete-action verb after \foreignlanguage{arabic}{لَنْ} \emph{lan}}{The a-state incomplete-action verb after لَنْ lan}}\label{the-a-state-incomplete-action-verb-after-ux644ux646-lan}
...
\section{\texorpdfstring{The a-state incomplete-action verb after \foreignlanguage{arabic}{إِذَنْ} \emph{ʾid͡han}}{The a-state incomplete-action verb after إِذَنْ ʾid͡han}}\label{the-a-state-incomplete-action-verb-after-ux625ux630ux646-eipan}

TODO

\section{\texorpdfstring{The a-state incomplete-action verb after \foreignlanguage{arabic}{وَ}, \foreignlanguage{arabic}{فَ}, \foreignlanguage{arabic}{أَوْ}, and \foreignlanguage{arabic}{ثُمَّ}}{The a-state incomplete-action verb after وَ, فَ, أَوْ, and ثُمَّ}}\label{the-a-state-incomplete-action-verb-after-ux648-ux641-ux623ux648-and-ux62bux645}
...
\foreignlanguage{arabic}{وَ}, \foreignlanguage{arabic}{فَ}, \foreignlanguage{arabic}{أَوْ}, and \foreignlanguage{arabic}{ثُمَّ} also have special meanings that can cause the following incomplete-action verb to be in the a-state. This is discussed in more detail in chapter TODO.
...
\subsection{\texorpdfstring{With \foreignlanguage{arabic}{لِ} for indirect commands}{With لِ for indirect commands}}\label{indirect-commands}
...
\tightlist
\item
  Seated on an ʾalif: \foreignlanguage{arabic}{أ}
\item
  Seated on an wāw: \foreignlanguage{arabic}{ؤ}
\item
  Seated on an yāʾ: \foreignlanguage{arabic}{ئ}
\item
...
\begin{itemize}
\item
  When unseated \emph{hamzaḧ} is followed by an ʾalif: \foreignlanguage{arabic}{ءا}, the combination of \emph{hamzaḧ} and ʾalif is conventionally written as \foreignlanguage{arabic}{آ} as long as the ʾalif is not a suffix or part of a suffix. When the ʾalif is a suffix or part of a suffix then we will write \foreignlanguage{arabic}{ءا}, not \foreignlanguage{arabic}{آ}.
\item
  When unseated \emph{hamzaḧ} comes between two letters that are joined, then it is written above the line that joins them, for example: \foreignlanguage{arabic}{خَطِيـَٔةٌ} \emph{k͡haṭīʾatun}. In this word, the yāʾ \foreignlanguage{arabic}{ي} joins to the looped tāʾ \foreignlanguage{arabic}{ة}. So the unseated \emph{hamzaḧ} is written above the joining line like so \foreignlanguage{arabic}{ــٔـ}.
\item
  When \emph{hamzaḧ} is seated on ʾalif, if it has an \emph{i}-mark, it is written below the ʾalif: \foreignlanguage{arabic}{إِ}. Otherwise, it is written above the ʾalif: \foreignlanguage{arabic}{أَ}, \foreignlanguage{arabic}{أُ}, \foreignlanguage{arabic}{أْ}.
\item
  When \emph{hamzaḧ} is seated on yāʾ \foreignlanguage{arabic}{ئ} the dots of the yāʾ are no longer written. Here's how it will appear in different positions:
...
\item
  ...
  \tightlist
  \item
    If the \emph{hamzaḧ} carries a long-\emph{ā} vowel, it is written unseated followed by an ʾalif and written as \foreignlanguage{arabic}{آ}, for example \foreignlanguage{arabic}{آمَنَ} \emph{ʾāmana}.
  \item
    If the \emph{hamzaḧ} carries any other vowel, it is written seated on an ʾalif, and is marked with the appropriated vowel mark, for example \foreignlanguage{arabic}{أَسْلَمَ} \emph{ʾaslama}, \foreignlanguage{arabic}{أُرِيدُ} \emph{ʾurīdu}, \foreignlanguage{arabic}{إِسْلَامُ} \emph{ʾislāmu}, \foreignlanguage{arabic}{إِيمَانُ} \emph{ʾīmānu}, \foreignlanguage{arabic}{أُوخِذَ} \emph{ʾūk͡hid͡ha}.
  \end{enumerate}
\item
  ...
  \item
    ...
    \item
      ...
      If the \emph{hamzaḧ} has an \emph{i}-mark it is written seated on yāʾ. Examples:
    ...
    \item
      ...
      If the \emph{hamzaḧ} has an \emph{i}-mark it is written seated on yāʾ. Example:
      \foreignlanguage{arabic}{سَائِلُ} \emph{sāʾilu}.\\
      If the \emph{hamzaḧ} has an \emph{u}-mark it is written seated on wāw. Example: \foreignlanguage{arabic}{تَسَاؤُلُ} \emph{tasāʾulu}.\\
    ...
    \end{enumerate}
  \item
    If the letter before the \emph{hamzaḧ} has a ø-mark and is not wāw or yāʾ, then:
    ...
    \item
      Otherwise, if the \emph{hamzaḧ} is originally at the middle of the word, then it is written seated on wāw if it has an \emph{u}-mark and yāʾ if it has an \emph{i}-mark. Examples: \foreignlanguage{arabic}{مَسْؤُولُ} \emph{masʾūlu}, \foreignlanguage{arabic}{تَرْئِيسُ} \emph{tarʾīsu}.\\
      If the \emph{hamzaḧ} has an \emph{a} mark then:\\
      If it is followed by a long-\emph{ā} vowel represented by an ʾalif, the \emph{hamzaḧ} is unseated followed by the ʾalif and the combination is written as \foreignlanguage{arabic}{آ}. Example: \foreignlanguage{arabic}{مِرْآةُ} \emph{mirʾātu}, \foreignlanguage{arabic}{ظَمْآنُ} \emph{ḍ͡hamʾānu}.\\
      Otherwise, if there is no ʾalif after the \emph{hamzaḧ}, the \emph{hamzaḧ} is written seated on ʾalif. Examples: \foreignlanguage{arabic}{مَسْأَلَةُ} \emph{masʾalatu}, , \foreignlanguage{arabic}{مَرْأَةُ} \emph{marʾatu}.
    \end{enumerate}
  \item
    If the \emph{hamzaḧ} has a ø-mark, then it is written seated on ʾalif if it has an \emph{a}-mark, wāw if it has an \emph{u}-mark, and yāʾ if it has an \emph{i}-mark. Examples: \foreignlanguage{arabic}{کَأْسُ} \emph{kaʾsu}, \foreignlanguage{arabic}{بِئْسَ} \emph{biʾsa}, \foreignlanguage{arabic}{سُؤْلَکَ} \emph{suʾlaka}.
  \item
    ...
    \tightlist
    \item
      If either vowel mark is an \emph{i}-mark then the \emph{hamzaḧ} will be written on a yāʾ. Examples: \foreignlanguage{arabic}{سُئِلَ} \emph{suʾila}, \foreignlanguage{arabic}{يَئِسَ} \emph{yaʾisa}, \foreignlanguage{arabic}{مُتَّکِئِينَ} \emph{muttakiʾīna}.
    \item
      If neither vowel mark is an \emph{i}-mark, and at least one of the vowel marks is a \emph{u}-mark, then the \emph{hamzaḧ} will be written on a wāw. Examples: \foreignlanguage{arabic}{سُؤَالُ} \emph{suʾālu}, \foreignlanguage{arabic}{رُؤُوسُ} \emph{ruʾūsu}, \foreignlanguage{arabic}{لُؤَيُّ} \emph{luʾayyu}.
    \item
      Otherwise, if both of the vowel marks are \emph{a}-marks, then:\\
      If the \emph{hamzaḧ} is followed by a long-\emph{ā} vowel represented by an ʾalif, the \emph{hamzaḧ} is written unseated. Examples: \foreignlanguage{arabic}{شَنَآنُ} \emph{s͡hanaʾānu}.\\
      Otherwise the \emph{hamzaḧ} will be written on an ʾalif. Examples: \foreignlanguage{arabic}{سَأَلَ} \emph{saʾala}, \foreignlanguage{arabic}{رَأَىٰ} \emph{raʾā}.
    \end{enumerate}
...
\item
  ...
  \item
    Otherwise, if the previous letter is a doubled wāw with an \emph{u}-mark, the \emph{hamzaḧ} will again be unseated. Example \foreignlanguage{arabic}{تَبَوُّءُ} \emph{tabawwuʾu}.
  \item
    ...
    \tightlist
    \item
      \emph{a}-mark, the \emph{hamzaḧ} is written seated on ʾalif. Example \foreignlanguage{arabic}{يَهْدَأُ} \emph{yahdaʾu}, \foreignlanguage{arabic}{مُبْتَدَإِ} \emph{mubtadaʾi}.
    \item
      \emph{i}-mark, the \emph{hamzaḧ} is written seated on yāʾ. Example \foreignlanguage{arabic}{يُهَدِّئُ} \emph{yuhaddiʾu}, \foreignlanguage{arabic}{سَيِّئُ} \emph{sayyiʾu}.
    \item
      \emph{u}-mark, the \emph{hamzaḧ} is written seated on wāw. Example \foreignlanguage{arabic}{بَطُؤَ} \emph{baṭuʾa}.
    \end{enumerate}
  \end{enumerate}
\end{enumerate}
