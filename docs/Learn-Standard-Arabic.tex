...
By the way, this \foreignlanguage{arabic}{لِ}, which is used with a-state incomplete-action verbs, is different from
the \foreignlanguage{arabic}{لِ} we learned in section~\ref{indirect-commands}, which is used with the ø-state incomplete-action verbs for indirect commands.
...
As with \foreignlanguage{arabic}{کَيْ}, \foreignlanguage{arabic}{أَنْ} must be not be expressed while its meaning is retained. Examples:

\foreignlanguage{arabic}{يَأْکُلُ حَتَّىٰ يَشْبَعَ.} (a-state)\\
\enquote{He eats for the result that he be sated.}

\foreignlanguage{arabic}{يَأْکُلُ حَتَّىٰ يَشْبَعُ.} (u-state)
