...
1. When [حَتَّىٰ]{.ar} is used to indicate an extreme point at which the action of the following verb would, or is meant to, occur. For example,
   ...
   It is noteworthy that the use of [حَتَّىٰ]{.ar}, here, implies only that the following action is meant, or at the point of being expected, to occur. It doesn't actually state that the action actually occured, for something may have prevented it from ocurrding in reality.
   ...
   In fact, 
   [حَتَّىٰ]{.ar},
   here, can be considered synonymous to [إِلَىٰ أَنْ]{.ar}. So the above two examples can be considered similar in meaning to:
...
Context would be needed to determine which meaning or whether one or both meanings are intended.
...
There is a modern tendency to use the words [Parf makan]{.trn2} [هُنَاكَ]{.ar} and the [majhUl]{.trn2} verb [يُوجَدُ]{.ar} as direct equivalents of the English "there" and "exists".
So one might find:
...
Our above directive to avoid the use of [يوجد]{.ar} to mean "exists" holds for dialogue, which is the original use of language.
...
So, [يوجد]{.ar} and its [ism mafeUl]{.trn2} [موجود]{.ar} may be used when needing to discuss the existence of something in a scientific text.
But such usage should be restricted to its domain, and should not, ideally, spill over to dialogue.

While this concession can be granted to [يوجد]{.ar}, we find no such justifying circumstance for the existential [هناك]{.ar}.
