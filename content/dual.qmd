...
There are some nouns that end with an [alif]{.trn2} before the [ة]{.ar}, like [فَتَاة]{.ar} [fatAh]{.trn} "a young woman". We will learn how to dualize these nouns later, if [#allAh]{.trn2} wills.
...
## Dual [na3t]{.subs}s and [manut]{.subs}s in descriptive noun-phrases

We learned that when an [sifah]{.subs} is a [na3t]{.subs} in an descriptive noun-phrase, then it matches the [manut]{.subs} in definiteness, state, and gender.  For example:

[ذَهَبْتُ إِلَى ٱلْمَدِينَةِ ٱلْقَدِيمَةِ.]{.ar}  
[pahabtu Eila -lmadInati -lqadImah.]{.trn}  
"I went to the old city."

To this we add that the [na3t]{.subs} shall also match the [manut]{.subs} in number. So if the [manut]{.subs} is a dual then the [sifah]{.subs} [na3t]{.subs} shall be dualzed to match it. Examples:
