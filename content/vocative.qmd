...
There are different rules regarding the case endings of the noun following [يَا]{.ar} [yA]{.trn} and we will describe them in the following sections.

## Calling out to specific persons

### Using single word personal names

When a specific person is called out to, and the name used to call him consists of a single word, then that word shall be in the u-case. The sentence above is an example of this rule where the name [زَينَبُ]{.ar} [zaynabu]{.trn} "Zaynab" is in the u-case.

If the word would be [tanwIn]{.trn2}ed, then the [tanwIn]{.trn2} is dropped. So, for example, the name [زَيْدٌ]{.ar} [zaydun]{.trn} "Zayd" usually is [tanwIn]{.trn2}ed. But when used for being called out to, the [tanwIn]{.trn2} is dropped and it becomes:
...
Note how the word [رِجَالُ]{.ar} [rijAlu]{.trn} "men" is not [tanwIn]{.trn2}ed because the word is used to call out to the specific persons.
...
In this case, then instead of the u-case, the word is put into the a-case. Furthermore, the [tanwIn]{.trn2}, if any, is preserved. Examples:
...
If, instead of a personal name, a noun phrase consisting of multiple words is used to call out to a person, then in this case as well, the first noun shall be in the a-case. Examples:
...
When multiple words are used to call out to a person, the second word in the noun-chain may be a pronoun. Here too, the first noun shall be in the a-case. Examples:
...
If an unspecified person or persons are being called out, then the word used to call out is put into the a-case. Furthermore, the [tanwIn]{.trn2}, if any, is preserved. So if you want to address any unspecific Muslim, you will say:
...
If multiple words are used, whether or not the person called out to is specific or unspecified, then too the first noun is put in the a-case.
...
Note that even when [يَا]{.ar} [yA]{.trn} is ommitted the name [زَيْدُ]{.ar} [zaydu]{.trn} "Zayd" is in the u-case without any [tanwIn]{.trn2}.
...
When calling someone with the pronoun "my", for example "O my people", it is common to shorten the attached pronoun [ي]{.ar} [I]{.trn} "my" to an [kasrah]{.trn2} [◌ِ]{.ar}. So while the following is permissible,
