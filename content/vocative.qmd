...
In this case, then instead of the u-state, the word is put into the a-state. Furthermore, the [tanwIn]{.trn2}, if any, is preserved. Examples:
...
If an unspecified person or persons are being called out, then the word used to call out is put into the a-state. Furthermore, the [tanwIn]{.trn2}, if any, is preserved. So if you want to address any unspecific Muslim, you will say:
...
Note that even when [يَا]{.ar} [yA]{.trn} is ommitted the name [زَيْدُ]{.ar} [zaydu]{.trn} "Zayd" is in the u-state without any [tanwIn]{.trn2}.
