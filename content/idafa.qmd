...
The word "[idafa]{.subs}" means the addition of a new _annexed_ item to an existing _base_ item. We use the term _[idafa]{.subs}_ in Arabic grammar when an _annexe_ noun is annexed to a _base_ noun by being placed right before it. Here is an example of an [idafa]{.subs}:
...
\draw [decoration={brace}, decorate] 
   (word-1.north west) -- (word-1.north east)
      node [midway, rotate=90, anchor=south, right=0ex] {\scriptsize [mudafil]{.subs}};
...
1. The _annexe_ noun: This is the first noun in the [idafa]{.subs}.
1. The _base_ noun: This is the second noun in the [idafa]{.subs}.
...
-->
## State of the annexe and [mudafil]{.subs}s
...
The definiteness of the [mudafil]{.subs} then determines the definiteness of the [idafa]{.subs}. If the second noun is definite, then both the annexe and the [mudafil]{.subs}s be definite. And if the [mudafil]{.subs} is indefinite, then both the nouns in the chain will be indefinite. 
...
Now, you might be foreseeing a problem. What if the [mudaf]{.subs} and the [mudafil]{.subs} have the same gender, and the annexe too is in the i-state? For example, in the sentence:
