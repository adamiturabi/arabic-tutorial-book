...
As you can see, the noun is [tanwIn]{.trn2}ed when it is indefinite, and also, the vowel mark on the last letter changes for each state that the noun is in. These kinds of nouns are called _fully-flexible_ nouns. They are by far the most common type of noun.
...
As you can see, when [صَحْرَاء]{.ar} [SaHrAE]{.trn} is indefinite, it is not [tanwIn]{.trn2}ed. Also, when it is indefinite and in the i-state, the vowel mark on its final letter is not [◌ِ]{.ar}, as you might expect but [◌َ]{.ar} . And so the noun looks identical in the a-state and i-state when it is indefinite.
...
2. When indefinite and in the i-state, a semi-flexible noun's final letter does not have an [kasrah]{.trn2}. Instead it shall have an [fatHah]{.trn2}, just like when it is in the a-state.
...
is also a attributee, whose attribute is [وَاسِعَةٍ]{.ar} [wAsieatin]{.trn} "wide". The final vowel mark [◌َ]{.ar} on the attributee [صَحْرَاءَ]{.ar} [SaHrAEa]{.trn} has no effect on the final vowel mark on the attribute [وَاسِعَةٍ]{.ar} [wAsieatin]{.trn} "wide". All that matters in this regard is the state of the attributee. 
...
Note, again how the attribute [جَمِيلَة]{.ar} [jamIlah]{.trn} is feminine and in the u-state, in order to match the gender and state of the attributee ^2^[ذِكْرَىٰ]{.ar} [pikrA]{.trn}^2^.
