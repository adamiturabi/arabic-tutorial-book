...
Sentence word order is usually pretty flexible. For stylistic reasons, it is permissible for a [mafulb]{.term} to precede the [fail]{.term}.
For example,
...
Their origin as verbs will be apparent when we study [mudarie]{.term} verbs.
...
So far we have only dealt with proper nouns that are single words. There are some proper nouns that may be formed from two words that are in an [idafa]{.term}. These belong to different categories:
...
As usual, the [mudafil]{.term} shall always be in the i-state. And the state of the [mudaf]{.term} [عَبْد]{.ar} is variable, depending on it's function in the sentence. Example:
...
(Note that the second [بْنِ]{.ar} is in the i-state to match the state of the [mudaf]{.term} [عَبْدِ]{.ar} in [عَبْدِ ٱللّـٰه]{.ar}.)
...
If the tribe name is the [fail]{.term} of a verb then it is usually treated as singular feminine.
...
Some titles are formed from [idafa]{.term}s. Examples:
...
In Arabic, the titles for these objects usually don't occur as replacees as they do for persons. Rather, the title is annexed to the proper noun in an [idafa]{.term}. Examples:
...
(Note how the second [بْنِ]{.ar} is in the i-state because it is a replacement of [عُمَرَ]{.ar} which is in the i-state because it is a [mudafil]{.term} of the first [بْنُ]{.ar}.)
