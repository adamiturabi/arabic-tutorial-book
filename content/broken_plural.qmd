...
Most nouns in Arabic are _fully-flexible_. This means that, when indefinite, they are [tanwIn]{.trn2}ed and the indefinite i-state is shown by a [tanwIn]{.trn2}ed [kasrah]{.trn2} [◌ٍ]{.ar} at the end of the noun. For example, [رَجُل]{.ar} [rajul]{.trn} "a man" and [بَيْت]{.ar} [bayt]{.trn} "a house" are fully-flexible nouns. So, you can see, below, that they are [tanwIn]{.trn2}ed, and the indefinite i-state is indicated by a [tanwIn]{.trn2}ed [kasrah]{.trn2} [◌ٍ]{.ar}:

[ذَهَبَ رَجُلٌ إِلَىٰ بَيْتٍ.]{.ar}  
[pahaba rajulun EilA bayt.]{.trn}  
"A man went to a house."

Some nouns are _semi-flexible_. This means that they are not [tanwIn]{.trn2}ed, and also, the indefinite i-state is indicated by an [fatHah]{.trn2} [◌َ]{.ar}. Examples of such nouns are:
...
+ The letters [ء]{.ar}, [ا]{.ar}, [و]{.ar}, and [ي]{.ar} are considered _weak_ letters. Words that one or more these weak letters in their roots are called _defective_ words. We will discuss defective words more completely in later chapters, if [#allAh]{.trn2} wills. For now, we will note the following:
  ...
  + A [ي]{.ar} at the end of a word, in some states, gets omitted  and replaced by a [tanwIn]{.trn2}ed [kasrah]{.trn2} [◌ٍ]{.ar} on the preceding letter. This happens even when the [ي]{.ar} is extrinsic to the root, and even if the word is semi-flexible (and thus would not normally be [tanwIn]{.trn2}ed). Examples:
...
Consistent with what we have learned so far, when the attributee in a noun-phrase is plural, then the attribute comes after it, and matches it in state, definiteness, gender, and number.

Either or both of the attribute and the attributee may be sound plurals or broken plurals.
...
### Plurals of intelligent beings in nounal sentences

If the subject of a sentence is a plural denoting intelligent beings then the info typically matches it in being a plural. This is especially the case if the info is an adjectival noun. For example:
...
Sometimes the info may not match the subject in plurality because of the meaning of the sentence. For example:
...
(Note how ^2^[أَشْيَاء]{.ar} is indefinite but is not [tanwIn]{.trn2}ed. This is because it is irregularly semi-flexible.)

By the way, this rule only applies to adjectival nouns in the attributee or the info. A common noun in the attribute or info will continue match the attributee or subject in gender and number.
...
Similarly, if an adjectival noun connoting a non-intelligent being is used not as a attribute or an info in a sentence, then it should be pluralized to indicate plurality.
