...
This construction of the verb is called the _[passive]{.term} verb_ construction.

## Forming the [passive]{.term} verb
...
The pattern of [passive]{.term} verb for form 1 verbs is always:

[فُعِلَ يُفْعَلُ]{.ar}  
[fueila yufealu]{.trn}

This is regardless of the vowel on the middle root letter in the known-doer verb.
If there is more than one verb from the same root, with different middle root letter vowels for the known-doer verb, then they will share the same [passive]{.term} verb. For example, the known-doer verbs: [حَسَبَ يَحْسُبُ]{.ar} "to reckon" and [حَسِبَ يَحْسِبُ]{.ar} "to deem" share the same [passive]{.term} verb: [حُسِبَ يُحْسَبُ]{.ar}.

The same doer pronouns are used for the [passive]{.term} verb as for the known-doer verb.
And the [mudarie]{.term} [passive]{.term} verb has the same three states as the [mudarie]{.term} known-doer verb.
For example:

+ [فُعِلُوا]{.ar} [fueilU]{.trn} "they~3m~" ([madi]{.term})
+ [تُفْعَلَانِ]{.ar} [tufealAni]{.trn} "you~2~" or "they~2f~" (u-state [mudarie]{.term})
+ [أُفْعَلَ]{.ar} [Eufeala]{.trn} "I" (a-state [mudarie]{.term})
+ etc.
...
Consider again this sentence with an [passive]{.term} verb construction:
...
Note that the noun [ٱَلْمَاءُ]{.ar} [EalmAEu]{.trn} "the water" is in the u-state. This is because, in the absence of the doer, the [mafulb]{.term} of the known-doer verb ([ٱَلْمَاءَ]{.ar}) is deputized to take doer's place in the u-state. It is then known as the _deputy doer_.

The doer pronouns for the [passive]{.term} verb shall therefore match the gender and number of the deputy doer. Here are some examples:
...
### The deputy doer for multiple [mafulb]{.term}s

Some verbs, in the known-doer construction, take multiple [mafulb]{.term}. These verbs can be classified into two classes:
<!--Wright vol. ii., p. 47D, sect. 24-->

a. Verbs that cause the first [mafulb]{.term} to be affected by, or asked for, the second [mafulb]{.term}. Examples:
...
When such verbs are converted to the [passive]{.term} verb construction, then one, and only one, [mafulb]{.term} shall be chosen to be the deputy doer. It is generally preferred to make the first [mafulb]{.term} the deputy-doer, and leave the second [mafulb]{.term}, as is, in the a-state.
Here are the same sentences in the [passive]{.term} construction:
...
When verbs are used without a [mafulb]{.term}, then their [passive]{.term} construction gives in an impersonal meaning. There are a few such usages that we will explain in the following subsections.

### With prepositional phrases

Some verbs take no [mafulb]{.term}s, but are used with prepositional phrases. For example,

[جَلَسَ ٱلنَّاسُ عَلَى ٱلْأَرْضِ.]{.ar}  
"The people sat upon the ground."

Other verbs, which can take a [mafulb]{.term}, may be used without one, and again with a prepositional phrase instead. For example:

[كَتَبَ ٱلْكَاتِبُ بِٱلْقَلَمِ.]{.ar}  
"The scribe wrote with the pen."

When such sentences are converted to the [passive]{.term} verb construction then the prepositional phrase may be taken as the deputy doer. 
...
Other verbs don't take a [mafulb]{.term} but may be used with an adverb of time or place in the a-state. 
...
When this sentence is converted to an [passive]{.term} construction then the adverb of time or place can be taken as the deputy doer in the u-state:
...
### With the [masdar]{.term}

The [masdar]{.term}, because it is the action being done may be considered a kind of [mafulb]{.term}. 
(We will study the use of the [masdar]{.term} as a [maful]{.term} in chapter\ \@ref(absolute-doee), if [_allAh]{.trn2} wills.)
For example:
...
When such sentences are converted to the [passive]{.term} verb construction, then the [masdar]{.term} may be taken as the deputy doer in the u-state.
...
When a prepositional phrase, or an adverb of time or place, or a [masdar]{.term} is to be used as a deputy doer in an [passive]{.term} verb construction, then it is required that they be specialized in meaning, and not used in a general sense. So for example, if we have the sentence:

[سَهَرَتِ ٱلْأُمُّ لَيلَةً.]{.ar}  
"The mother stayed up a night."

then because of the non-specialized meaning of [لَيْلَةً]{.ar}, such a sentence is typically not suitable for being converted to a [passive]{.term} verb construction.

### Choosing the deputy doer

If a sentence is to be converted to an [passive]{.term} verb construction and it has more than one of the following:

+ a [mafulb]{.term}
+ a specialized prepositional phrase
+ a specialized adverb of time or place
+ a specialized [masdar]{.term}

Then only one shall be chosen as the deputy doer. If a [mafulb]{.term} exists, then it is typically chosen. If a [mafulb]{.term} does not exist, then the one desired to be emphasized may be chosen as the deputy doer. For example:
...
## States of the [mudarie]{.term} [passive]{.term} verb

The 
[mudarie]{.term} verb [passive]{.term} verb
has three states, just like the
[mudarie]{.term} verb known-doer verb:
...
(Note that [يُغْفَرَ]{.ar} has a singular masculine deputy doer because there is no [mafulb]{.term}.)
...
+ With [لِ]{.ar} for indirect commands:
  ...
  There is no verb of command for [passive]{.term} verbs. So while indirect commands are typically not used for the addressee participant for known-doer verbs (see section\ \@ref(indirect-commands)), they are the only way to issue commands for the addressee participant in the [passive]{.term} verb construction:

  [لِتُنْصَرُوا!]{.ar}  
  "Let you~3m~ be aided!"

## Usage of the [passive]{.term} verb

There are cases where it is permissible to 
use of the [passive]{.term} verb construction,
and cases where it is _not_ permissible to 
use of the [passive]{.term} verb construction. We will explain them below:

### Permissible use of the [passive]{.term} verb

There can be a number of reasons why a speaker is forced to, or chooses to, use the [passive]{.term} verb construction. Among these reasons are:

a. When the doer is actually unknown to the speaker. This is the classic use-case, as indicated by the name _[passive]{.term} verb_. (But, as we shall see below, it is not the only use case.) So, if we say:

   [كُسِرَتِ ٱلزُّجَاجَةُ.]{.ar}  
   "The glass was broken."

   then, it may be that we don't know who broke the glass, and that is why we are expressing it in the [passive]{.term} verb construction.
...
e. In order to glorify the doer. So we might the [passive]{.term} verb construction to say:
...
These reasons are not mutually exclusive, and sometimes the [passive]{.term} verb construction is used for a combination of them.

### Impermissible use of the [passive]{.term} verb

The [passive]{.term} verb may not be used when the doer is mentioned with the verb. This is different from English which can use the word "by" to indicate the doer in a passive voice construction, as in: "The book was written by Zayd.". Such a sentence, in Arabic can only be expressed with the known-doer verb construction:
