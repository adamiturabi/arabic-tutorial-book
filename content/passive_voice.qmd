...
And the passive [muDArie]{.trn2} verb has the same three cases as the active [muDArie]{.trn2} verb.
For example:

+ [فُعِلُوا]{.ar} [fueilU]{.trn} "they~3m~" ([mADI]{.trn2})
+ [تُفْعَلَانِ]{.ar} [tufealAni]{.trn} "you~2~" or "they~2f~" (u-case [muDArie]{.trn2})
+ [أُفْعَلَ]{.ar} [Eufeala]{.trn} "I" (a-case [muDArie]{.trn2})
+ etc.
...
Note that the noun [ٱَلْمَاءُ]{.ar} [EalmAEu]{.trn} "the water" is in the u-case. This is because, in the absence of the doer, the direct doee of the active verb ([ٱَلْمَاءَ]{.ar}) is deputized to take doer's place in the u-case. It is then known as the _deputy doer_.
...
When such verbs are converted to the passive verb construction, then one, and only one, direct doee shall be chosen to be the deputy doer. It is generally preferred to make the first direct doee the deputy-doer, and leave the second direct doee, as is, in the a-case.
...
However, the preposition causes the noun following it to remain in the i-case. So the deputy doer is not indicated by an apparent [Dammah]{.trn2} (or by the other indicators of the u-case).
...
Other verbs don't take a direct doee but may be used with an adverb of time or place in the a-case. 
...
\draw [decoration={brace, mirror}, decorate] 
   (word-1.south west) -- (word-1.south east)
      node [midway, below] {\scriptsize a-case adverb of time};
...
When this sentence is converted to an passive construction then the adverb of time or place can be taken as the deputy doer in the u-case:

[سُهِرَتْ لَيلَةُ ٱلْقَدْرِ.]{.ar}  
"The Night of Power was stayed up [in]."

Note the u-case of [لَيْلَةُ]{.ar} [laylatu]{.trn} as the deputy doer.
...
When such sentences are converted to the passive verb construction, then the [maSdar]{.trn2} may be taken as the deputy doer in the u-case.
...
In the above sentence the prepositional phrase [فِي ٱلْمَسْجِدِ]{.ar} was chosen as the deputy doer, and the adverb of time [لَيْلَةَ]{.ar} remains, as is, in the a-case.

## Cases of the passive [muDArie]{.trn2} verb

The 
passive [muDArie]{.trn2} verb
has three cases, just like the
active [muDArie]{.trn2} verb:
the u-case, the a-case, and the ø-case.
The three cases are used in the same circumstances, and with the same rules that we have already learned.

So the u-case verb is used for making plain statements:
...
The a-case verb is used for expressing wish or purpose:
...
Similarly, the ø-case verb is used in the following cases:
