...
If there is more than one verb from the same root, with different middle root letter vowels for the known-doer verb, then they will share the same [passive]{.subs} verb. For example, the known-doer verbs: [حَسَبَ يَحْسُبُ]{.ar} "to reckon" and [حَسِبَ يَحْسِبُ]{.ar} "to deem" share the same [passive]{.subs} verb: [حُسِبَ يُحْسَبُ]{.ar}.

The same doer pronouns are used for the [passive]{.subs} verb as for the known-doer verb.
And the [mudarie]{.subs} [passive]{.subs} verb has the same three states as the [mudarie]{.subs} known-doer verb.
For example:

+ [فُعِلُوا]{.ar} [fueilU]{.trn} "they~3m~" ([madi]{.subs})
+ [تُفْعَلَانِ]{.ar} [tufealAni]{.trn} "you~2~" or "they~2f~" (u-state [mudarie]{.subs})
+ [أُفْعَلَ]{.ar} [Eufeala]{.trn} "I" (a-state [mudarie]{.subs})
+ etc.
...
Note that the noun [ٱَلْمَاءُ]{.ar} [EalmAEu]{.trn} "the water" is in the u-state. This is because, in the absence of the doer, the [mafulb]{.subs} of the known-doer verb ([ٱَلْمَاءَ]{.ar}) is deputized to take doer's place in the u-state. It is then known as the _deputy doer_.

The doer pronouns for the [passive]{.subs} verb shall therefore match the gender and number of the deputy doer. Here are some examples:
...
### The deputy doer for multiple [mafulb]{.subs}s

Some verbs, in the known-doer construction, take multiple [mafulb]{.subs}. These verbs can be classified into two classes:
...
When such verbs are converted to the [passive]{.subs} verb construction, then one, and only one, [mafulb]{.subs} shall be chosen to be the deputy doer. It is generally preferred to make the first [mafulb]{.subs} the deputy-doer, and leave the second [mafulb]{.subs}, as is, in the a-state.
...
When such sentences are converted to the [passive]{.subs} verb construction then the prepositional phrase may be taken as the deputy doer. 
However, the preposition causes the noun following it to remain in the i-state. So the deputy doer is not indicated by an apparent [umark]{.subs} (or by the other indicators of the u-state).
...
Other verbs don't take a [mafulb]{.subs} but may be used with an adverb of time or place in the a-state. 
...
When this sentence is converted to an [passive]{.subs} construction then the adverb of time or place can be taken as the deputy doer in the u-state:
...
(We will study the use of the [masdar]{.subs} as a [maful]{.subs} in chapter\ \@ref(absolute-doee), if [_allAh]{.trn2} wills.)
For example:
...
When such sentences are converted to the [passive]{.subs} verb construction, then the [masdar]{.subs} may be taken as the deputy doer in the u-state.
...
When a prepositional phrase, or an adverb of time or place, or a [masdar]{.subs} is to be used as a deputy doer in an [passive]{.subs} verb construction, then it is required that they be specialized in meaning, and not used in a general sense. So for example, if we have the sentence:
...
Then only one shall be chosen as the deputy doer. If a [mafulb]{.subs} exists, then it is typically chosen. If a [mafulb]{.subs} does not exist, then the one desired to be emphasized may be chosen as the deputy doer. For example:
...
## States of the [mudarie]{.subs} [passive]{.subs} verb

The 
[mudarie]{.subs} verb [passive]{.subs} verb
has three states, just like the
[mudarie]{.subs} verb known-doer verb:
...
(Note that [يُغْفَرَ]{.ar} has a singular masculine deputy doer because there is no [mafulb]{.subs}.)
...
+ With [لِ]{.ar} for indirect commands:
  ...
  There is no verb of command for [passive]{.subs} verbs. So while indirect commands are typically not used for the addressee participant for known-doer verbs (see section\ \@ref(indirect-commands)), they are the only way to issue commands for the addressee participant in the [passive]{.subs} verb construction:
...
The [passive]{.subs} verb may not be used when the doer is mentioned with the verb. This is different from English which can use the word "by" to indicate the doer in a passive voice construction, as in: "The book was written by Zayd.". Such a sentence, in Arabic can only be expressed with the known-doer verb construction:
