...
1. It can be used to describe a noun in an attributee-attribute phrase. For example:

   "a big car"

2. The adjective "big" can also be used as the info of a sentence, describing the subject noun. For example:
...
## Terminology: the _attribute_ and the _attributee_

We take this opportunity to introduce some grammatical terminology. The attributee-attribute phrase "a big car" consists of two parts:

i. The adjective "big". It is describing the car. We will call it the _attribute_ in the noun-phrase.
ii. The common noun "a car": It is being described by the attribute. We will call it the _attributee_.
...
We will reserve this terminology of _attribute_ and _attributee_ only for the noun and adjective in an attributee-attribute phrase. So we won't use this terminology for the sentence: "The car is big."

Instead, here we will continue to use the existing terminology of _subject_ and _info_. The definite noun "the car" is the subject of this sentence, and the adjective "big" is the info.
...
In the above sentence the adjective "antique" is a attribute and is describing the noun "table".

It can also be used as the info of a sentence, just like an adjective. For example:
...
## The attribute and the attributee in attributee-attribute phrases

Let's learn how attributee-attribute phrases are formed in Arabic.

We learned in section\ \@ref(terminology-the-attribute-and-the-attributee)
above that attributee-attribute phrases consist of a attribute and a attributee. 

In English attributee-attribute phrases, like "the small house", the adjective attribute ("small") comes before the attributee ("house"). Also, only one definite article ("the") is used before the entire noun-phrase.

Here is the equivalent Arabic attributee-attribute phrase:
...
+ The adjectival noun attribute [ٱَلصَّغِير]{.ar} [EaSSagIr]{.trn} "the small one~m~" comes after the attributee [ٱَلْبَيْت]{.ar} [Ealbayt]{.trn} "the house".
+ Both the adjectival noun attribute [ٱَلصَّغِير]{.ar} [EaSSagIr]{.trn} "the small one~m~" and the attributee [ٱَلْبَيْت]{.ar} [Ealbayt]{.trn} "the house" get the definite article [ٱَلْ]{.ar} "the".
+ The adjectival noun attribute [ٱَلصَّغِير]{.ar} [EaSSagIr]{.trn} "the small one~m~" is genderized to match the attributee [ٱَلْبَيْت]{.ar} [Ealbayt]{.trn} "the house" in gender.
+ The adjectival noun attribute [ٱَلصَّغِير]{.ar} [EaSSagIr]{.trn} "the small one~m~" matches the attributee [ٱَلْبَيْت]{.ar} [Ealbayt]{.trn} "the house" in case. In this example, they were both in the u-case but we will see examples in the other cases as well.
+ The word-for-word equivalence of the above attributee-attribute phrase is "the small-one house" but we will usually give the more natural translation: "the small house"
...
This sentence has three attributee-attribute phrases. We will analyze each one individually:

i. [ٱَلْجَارِيَةُ ٱلصَّغِيرَةُ]{.ar}  
   [EaljAriyatu -SSagIratu]{.trn}  
   "the little girl"

   In this phrase the definite feminine noun [ٱلْجَارِيَةُ]{.ar} [EaljAriyatu]{.trn} is the doer of the verb [أَخَذَ]{.ar} [Eaxapa]{.trn} "took". Therefore it is in the u-case. It is also the attributee in the attributee-attribute phrase. Its attribute [ٱَلصَّغِيرَةُ]{.ar} [EaSSagIratu]{.trn} follows the attributee and is made to match the attributee in case (u-case), gender (feminine), and definiteness (definite).

ii. [كِتَابًا جَدِيدًا]{.ar}  
    [kitAban jadIdan]{.trn}  
    "a new book"

    In this phrase the indefinite masculine noun [كِتَابًا]{.ar} [kitAban]{.trn} is the direct doee of the verb [أَخَذَ]{.ar} [Eaxapa]{.trn} "took". Therefore it is in the a-case. It is also the attributee in the attributee-attribute phrase. Its attribute [جَدِيدًا]{.ar} [jadIdan]{.trn} follows the attributee and is made to match the attributee in case (a-case), gender (masculine), and definiteness (indefinite).

iii. [ٱَلْأُمِّ ٱلطَّيِّبَةِ]{.ar}  
     [EalEummi -TTayyibati]{.trn}  
     "the good mother"

      In this phrase the definite feminine noun [ٱلْأُمِّ]{.ar} [EalEummi]{.trn} is following the preposition [مِنْ]{.ar} [min]{.trn} "from". Therefore it is in the i-case. It is also the attributee in the attributee-attribute phrase. Its attribute [ٱَلطَيِّبَةِ]{.ar} [EaTTayyibati]{.trn} follows the attributee and is made to match the attributee in case (i-case), gender (feminine), and definiteness (definite). 

      Note carefully that the attribute matches the attributee in gender, not necessarily in having the same [ة]{.ar} ending. The feminine adjectival noun attribute [ٱَلطَيِّبَة]{.ar} [EaTTayyibah]{.trn} is still formed using the feminine marker [ة]{.ar}, despite the feminine attributee [ٱَلْأُمّ]{.ar} not having the [ة]{.ar} feminine marker.
...
If such a noun is a attributee, then we will prefer to match the attribute to the grammatical gender of the noun, not the physical gender of the person it is referring to. For example:
...
But the feminine adjectival noun
...
## The Adjectival noun as the info of a sentence

### Indefinite adjectival noun

Let's see how to use Arabic adjectival nouns as the info of a sentence.
...
[صَغِير]{.ar} [SagIr]{.trn} "a small one"
is used as the info of a sentence. Its indefiniteness and u-case is indicated by the [tanwIn]{.trn2}ed [Dammah]{.trn2} [◌ٌ]{.ar} on its end.

When an adjectival noun is the info of a sentence, then it shall be genderized to match the gender of the subject noun. The subject noun in this case ([ٱَلْبَيْت]{.ar}) is masculine. Therefore, the masculine adjectival noun ([صَغِير]{.ar}) is chosen.
...
Let's see if a definite adjectival noun can be used in the info. For example, we would like to say "The old tree is the big one."

The subject of the sentence is [ٱَلشَّجَرَةُ ٱلْقَدِيمَةُ]{.ar} [Eaccajaratu -lqadImuiatu]{.trn} "the old tree".
And the info is [ٱلْكَبِيرَةُ]{.ar} [EalkabIratu]{.trn} "the big one". When we put the two together we get:
...
The solution, too, is the same. We insert a detached pronoun, that matches the gender of the subject, between the subject and the info. So in order to get our intended meaning, we will say:
...
We have mentioned that adjectival nouns are just like other nouns that we have learned so far, in that they have gender, case, and definiteness. Can we then use
...
And the info is the feminine noun [حَسَنَة]{.ar} [Hasanah]{.trn} "a good deed".
Note that the info does not match the subject in gender. This is because it lost its genderizability since it is no longer acting as an adjectival noun "a good one~f~", but rather as the common noun "a good deed".
...
[ٱَلصَّدَقَة.]{.ar} 
[EaSSadaqah]{.trn} "charity" now matches the gender of the info
[حَسَنَة]{.ar} [Hasanah]{.trn}.
So now, technically, the info could be the adjectival noun, meaning "a good one~f~". So the sentence could mean:

"Charity is good."

Or the info could be the common noun, meaning "a good deed". Then the sentence would mean:

"Charity is a good deed."

Context would be needed to tell us which meaning is intended.

## Common-nouns used as attributes in a noun-phrase

Usually, adjectival nouns are used as the attribute in an attributee-attribute phrase. However, we also often find a common noun used as a attribute. For example,
...
This is discussed in more detail in section\ (participle-like adjective: entity noun crossover).
...
In an English attributee-attribute phrase, multiple attributes may describe the same attributee, without being separated by the word "and". For example, "The man is a famous Arab writer." In Arabic, we can do the same, except the attributees will be in the reverse order:
...
## A prepositional phrase separating the attribute from the attributee
...
<!--The prepositional phrase [مِنَ ٱلْمَكْتَبَةِ]{.ar} [mina -lmaktabati]{.trn} "from the library" is not a adjectival noun. Yet it still acts as a attribute for the noun [كِتَابٌ]{.ar} [kitAbun]{.trn} "a book", in that it is describing a quality of the book.
-->
If we want to add a adjectival noun as to describe "the book", we may add it either before or after the prepositional phrase attribute. Here are both examples as complete sentences:
...
<!--
## A prepositional phrase as the attribute
...
If we want to add a adjectival noun as an additional attribute, we may add it either before or after the prepositional phrase attribute. Here are both examples as complete sentences:
