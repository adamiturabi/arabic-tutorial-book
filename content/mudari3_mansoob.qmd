...
In 
chapter\ \@ref(u-state-stateful-verbs)
we mentioned that stateful verbs have three states (like nouns). 
...
We introduced the u-state stateful verb in
chapter\ \@ref(u-state-stateful-verbs).
In this chapter we will study the 
a-state
stateful verb.

The u-state stateful verb makes a plain statement.
The a-state stateful verb implies a wish or purpose.
The a-state stateful verb is used after the following articles:
...
## Forming the a-state stateful verb

Here is the u-state stateful verb for the singular masculine absentee participant doer "he":

[يَفْعَلُ]{.ar}  
[yafealu]{.trn}  
"he does"

Note that, because it is in the u-state, the its final letter ends with a [Dammah]{.trn2} [◌ُ]{.ar}.
...
This is done for all participants whose doer pronoun is invisible and u-state verb ends with a [Dammah]{.trn2} [◌ُ]{.ar}.
...
|Participant|Stateful verb doer pronoun|u-state stateful verb | a-state stateful verb|
|:---|:--|:---|:---|
...
stateful verb to be in the a-state.
...
### Basic usage of [أَنْ]{.ar} [Ean]{.trn} with the a-state stateful verb
...
[لَا]{.ar} can be used to negate the following a-state stateful verb.
...
[أَنْ]{.ar} must directly precede the following a-state stateful verb and must not be separated from it.
...
### [أَنْ]{.ar} with the stateless verb

[أَنْ]{.ar} may also occur before a stateless verb. Example:
...
<!--
By the way, this [لِ]{.ar}, which is used with a-state stateful verbs, is different from 
the [لِ]{.ar} we learned in section\ \@ref(indirect-commands), which is used with the ø-state stateful verbs for indirect commands.
-->

By the way, the grammatical equivalence of [أَنْ]{.ar} and a following a-state stateful verb with a [maSdar]{.trn2} applies also to when [لِ]{.ar} is used before (either an expressed or an implied) [أَنْ]{.ar}. So, for example, if we have a sentence:
...
There is a specific [لِ]{.ar}, called the [لِ]{.ar} of denial,  which is used with a-state stateful verbs and the verb [كَانَ]{.ar} that we will discuss in section (TODO in [كَانَ]{.ar} chapter).
...
It is also used before the a-state stateful verb.
The difference from [لِ]{.ar} is that, when [لِ]{.ar} is used with the a-state stateful verb, expressing or dropping the [أَنْ]{.ar} was optional.
...
By the way, 
[كَيْ]{.ar} and a following a-state stateful verb are not _directly_ replaced by
a [maSdar]{.trn2}.
...
Now
[رَأْس]{.ar}
is in the u-state because it is actually the subject of a new sentence, whose info is (an either expressed or implied) [أَكَلْتُهَا]{.ar} "I ate it".
[حَتَّىٰ]{.ar},
...
#### [حَتَّىٰ]{.ar} with a following a-state stateful verb

When
[حَتَّىٰ]{.ar}
is used with an expectation or purpose of a future action of the verb following it, then 
the verb following it is an a-state stateful verb. 
<!--
When the verb following
[حَتَّىٰ]{.ar}
signifies the expectation or purpose of a future action, then the verb used is an a-state stateful verb. 
-->
...
1. When [حَتَّىٰ]{.ar} is used to indicate an extreme point at which the action of the following verb would occur, or is meant to occur. Here, 
   ...
   Also, similar to the case of [كَيْ]{.ar}, there is an assumed (but mandatorily unexpressed) [أَنْ]{.ar} which is the real cause of the following stateful verb being in the a-state.
   In fact, 
   [حَتَّىٰ]{.ar},
...
2. When [حَتَّىٰ]{.ar} is used with the meaning "to such a purpose that". This is a similar meaning to [كَيْ]{.ar} "in order that".
   ...
   Again, there is an assumed (but mandatorily unexpressed) [أَنْ]{.ar} which is the real cause of the following stateful verb being in the a-state.
...
[حَتَّىٰ]{.ar}
in this case
may even be a stateless verb. For example:

[أَكَلْتُ ٱلطَّعَامَ حَتَّىٰ شَبِعْتُ.]{.ar}  
"I ate the food until I became full."

When used with a following stateful verb, the verb is put in the u-state and the meaning is that the action of the verb _before_
[حَتَّىٰ]{.ar}
...
Compare these examples with the corresponding ones in the previous sub-section that have an a-state stateful verb.


<!--The preposition 
[حَتَّىٰ]{.ar} [HattA]{.trn}
is unusual in that the stateful verb following it may either be in the a-state or the u-state, with slightly different meanings.
When the stateful verb following it is in the u-state then
[حَتَّىٰ]{.ar} gives the meaning "until".
And when the stateful verb following it is in the a-state then
...
[حَتَّىٰ]{.ar} 
is also used with stateless verbs. Example:

[أَكَلَ حَتَّىٰ شَبِعَ.]{.ar} (stateless, no [أَنْ]{.ar})  
"He ate until he was sated."
-->

## After [لَنْ]{.ar} [lan]{.trn}

[لَا]{.ar} and [أَنْ]{.ar} are combined to form [لَنْ]{.ar} [lan]{.trn} with the meaning "shall not". [لَنْ]{.ar} is used with the a-state stateful verb to emphatically negate the future.
...
occur after an a-state stateful verb, then 
a second a-state stateful verb
...
[وَ]{.ar}, [فَ]{.ar}, [أَوْ]{.ar}, and [ثُمَّ]{.ar} also cause the following stateful verb to be in the a-state in their own right, not simply as connectors. This is discussed in more detail in chapter TODO.
