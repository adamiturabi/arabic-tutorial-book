...
## Forming the ø-state stateful verb
...
|Participant|Stateful verb doer pronoun|u-state stateful verb | ø-state stateful verb|
|:---|:--|:---|:---|
...
+ The u-state and ø-state verbs for the masculine plural absentee and addressee participants have a final silent [alif]{.trn2}: 
...
<!--
## Uses of the ø-state stateful verb
...
Finally, the [لِ]{.ar} before a ø-state stateful verb for the indirect command is mandatory.
...
section\ \@ref(negating-stateless-verbs) that the stateless verb is negated using the particle [مَا]{.ar}. For example:
...
## Other uses of the ø-state stateful verb

The ø-state stateful verb is also used for _consequential actions_ and in _conditional statements_. We will deal with these in
...
Instead, the 
ø-state verb is used with [لَا]{.ar}
as described in
