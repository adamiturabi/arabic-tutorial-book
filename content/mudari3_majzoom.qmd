...
We have already studied the u-state of stateful verbs in 
chapter\ \@ref(u-state-stateful-verbs).
And we will defer the study of 
a-state of stateful verbs to
chapter\ \@ref(a-state-stateful-verbs).
...
## Forming the 0-state stateful verb
...
|Participant|Stateful verb doer pronoun|u-state stateful verb | 0-state stateful verb|
|:---|:--|:---|:---|
...
+ The u-state and 0-state verbs are the same for the feminine plural absentee and addressee participants: 
  + [يَفْعَلْنَ]{.ar} (they~3+,f~)
  + [تَفْعَلْنَ]{.ar} (you~3+,f~)
+ The u-state and 0-state verbs for the masculine plural absentee and addressee participants have a final silent [alif]{.trn2}: 
...
<!--
## Uses of the 0-state stateful verb

The u-state is the default state for stateful verbs. The 
0-state
...
Finally, the [لِ]{.ar} before a 0-state stateful verb for the indirect command is mandatory.
...
0-state. We have already seen in 
section\ \@ref(u-state-verb-negation-la)
that [لَا]{.ar} can be used to negate a u-state stateful verb for the present and future tense.
Example:
...
section\ \@ref(negating-perfect-verbs) that the perfect verb is negated using the particle [مَا]{.ar}. For example:
...
## Other uses of the 0-state stateful verb

The 0-state stateful verb is also used for _consequential actions_ and in _conditional statements_. We will deal with these in
...
Instead, the 
0-state verb is used with [لَا]{.ar}
as described in
