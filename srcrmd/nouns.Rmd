...
Note that we have divided the table above into two groups. The first group contains nouns that refer to refer to entities, without an obvious adjectival or verbal quality in the meaning, for example "human" "cat", etc. 
...
And [كَلْبَة]{.ar} is typically only used when the gender of the animal is relevant to the sentence.
...
This preference of the noun of one gender to refer to enties of either physical gender is case-by-case. For example,
Similarly, [طِفْل]{.ar} [Tifl]{.trn} (masc.) is commonly used to say "a child", regardless of whether the child is a boy or a girl. But [طِفْلَة]{.ar} [Tiflah]{.trn} "a female child" is fairly common too.
...
There aren't very many of such nouns. In fact, we have covered many of the common ones above. A good dictionary will also provide guidance in this regard.
...
There are other such exceptionAs.
