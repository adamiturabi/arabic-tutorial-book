...
Both expressions are, more or less, equivalent in meaning. They both establish a relation of _belonging_ between the two nouns: (i)\ "the boy", and (ii)\ "the book". The expressions state that the book _belongs_ to the boy.
...
\draw [decoration={brace, mirror}, decorate] 
   (word-1.south west) -- (word-1.south east)
      node [midway, below=1ex] {\footnotesize base};

\draw [decoration={brace, mirror}, decorate] 
   (word-2.south west) -- (word-2.south east)
      node [midway, below=1ex] {\footnotesize annexe};
...
The base noun in an annexation will always be in the i-state. The annexe noun may be in any state, depending on its function in the sentence. For example,
<!--
...
Referring back to the annexation expression we have been using so far:
...
The base noun [ٱَلْغُلَام]{.ar} "the boy" is definite because it is prefixed by [ٱَلْ]{.ar} "the".
The annexe noun [كِتَاب]{.ar} is not permitted to be prefixed by [ٱَلْ]{.ar} or have an [n]{.trn}-mark. 
Its definiteness is determined by the base noun. 
If the base noun is definite (as in this case) then the annexe noun shall also be definite.

If the base noun is indefinite (as below) then the annexe noun shall also be indefinite.
